...
export default function Welcome() {
  ...
  useEffect(() => {
    ...
    try {
      const payload = JSON.parse(atob(token.split(".")[1])); 
      const policy = payload.sub;
      setPolicyNumber(policy);
      fetch(`https://insuredperson-api-458668609912.us-central1.run.app/api/insuredpersons/${policy}`, {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
        ...
        <div className="card-body text-center">
         <h1 className="card-title mb-3 text-primary fw-bold">
          ...
          {policyNumber && (
            <div className="mb-4">
              <img
                src={`https://insuredperson-api-458668609912.us-central1.run.app/api/insuredpersons/profile-picture/${policyNumber}?t=${Date.now()}`}
                alt="Profile"
                style={{
                  width: "120px",
                  height: "120px",
