...
const workerBlob = new Blob([`importScripts("${window.location.origin}/pdf.worker.mjs");`], {
  type: "application/javascript",
});
pdfjs.GlobalWorkerOptions.workerSrc = URL.createObjectURL(workerBlob);

// Field mapping from uploaded file headers to formData keys
const fieldMapping = {
  FirstName: "firstName",
  LastName: "lastName",
  Age: "age",
...
export default function CreatePolicyHolder() {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (ext === "json") {
        ...
        extractedData = Array.isArray(jsonData) ? jsonData[0] : jsonData;
        ...
        extractedData = { content: pdfText };
      } else if (ext === "txt") {
        const text = await file.text();
        extractedData = { content: text };
      } else {
        alert("Unsupported file type. Upload JSON, CSV, XLSX, PDF, or TXT.");
        return;
      }

      // Map file headers to formData keys
      const normalizedData = {};
      Object.keys(extractedData).forEach((key) => {
        if (fieldMapping[key]) normalizedData[fieldMapping[key]] = extractedData[key];
      });

      setFormData((prev) => ({ ...prev, ...normalizedData }));
      setMessage(`Autofilled data from ${file.name}`);
    } catch (err) {
  ...
  const validateForm = () => {
    ...
    if (!formData.userId || !userIdRegex.test(formData.userId))
      newErrors.userId =
        "UserId must be 8+ chars long and contain uppercase, lowercase, digit & special char.";

    const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@!$*_])[A-Za-z\d@!$*_]{8,}$/;
    if (!formData.password || !passwordRegex.test(formData.password))
      newErrors.password =
        "Password must be 8+ chars long and contain uppercase, lowercase, digit & special char.";
    ...
    const phoneRegex = /^[2-9][0-9]{9}$/;
    if (!formData.phoneNumber || !phoneRegex.test(formData.phoneNumber))
      newErrors.phoneNumber = "Phone number must be 10 digits starting from 2-9.";
    ...
    ["street", "city", "state", "country"].forEach((f) => {
      if (!formData[f].trim()) newErrors[f] = `${f[0].toUpperCase() + f.slice(1)} is required.`;
    });
  ...
  const handleGoToProfile = () => navigate("/adminhome/welcome");
    ...
    <div className="card shadow-sm p-4 bg-white mx-auto" style={{ maxWidth: "80%" }}>
      ...
      {showForm && (
        <div className="mb-3 text-center">
          <input
            type="file"
            accept=".json,.csv,.xlsx,.pdf,.txt"
            onChange={handleFileUpload}
            className="form-control"
          />
          {loading && <p className="mt-2 text-muted">Processing file...</p>}
      ...
      {showForm ? (
        ...
        <div className="text-center">
          ...
          <div className="mt-4">
            <p>Do you want to create another policy holder?</p>
            <button className="btn btn-primary me-2" onClick={handleCreateAnother}>
              Yes
            </button>
