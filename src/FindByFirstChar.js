...
export default function FindByFirstChar() {
    ...
    try {
      ...
      if (response.ok) {
        if (data.data && data.data.length > 0) {
          ...
          setResults([]);
          setMessage(`No insured person found starting with: ${firstChar}`);
        }
      } else {
        setResults([]);
  ...
  return (
    <div className="p-4 text-center">
      <h2 className="mb-4 text-xl fw-bold text-primary border-bottom pb-2 text-center">
        Search Policy Holders by First Character
      </h2>
      ...
      >
        <input
          ...
          placeholder="Enter first character of first name"
          value={firstChar}
