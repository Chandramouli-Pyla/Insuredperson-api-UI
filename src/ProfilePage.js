...
export default function ProfilePage() {
  ...
  const handleProfilePicUpload = async () => {
    ...
    try {
      const response = await fetch(
        `https://insuredperson-api-458668609912.us-central1.run.app/api/insuredpersons/profile-picture/${policyNumber}`,
        {
          method: "PUT",
          body: formData,
        }
      ...
      {editingField === field ? (
        <>
          <input
            ...
            onChange={(e) => setFieldValue(e.target.value)}
          />
          <button onClick={() => handleSave(field)} style={styles.okBtn}>✅</button>
          <button onClick={() => setEditingField(null)} style={styles.cancelBtn}>❌</button>
        </>
      ) : (
        <>
          <span style={styles.value}>{profile[field] || "-"}</span>
      ...
      <div style={styles.card}>
        ...
        {policyNumber && (
          <div style={{ textAlign: "center", marginBottom: "20px" }}>
            <img
              src={`https://insuredperson-api-458668609912.us-central1.run.app/api/insuredpersons/profile-picture/${policyNumber}?t=${Date.now()}`} // ?t= to bust cache
              alt="Profile"
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          />
          <button onClick={handleProfilePicUpload} style={styles.okBtn}>
            Upload Picture
          </button>
        </div>
